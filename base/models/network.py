...
def name_parse(name):
    ...
    if pool_str in sota_pool_tpls:
        ...
        if len(strides)==4:
            ...
            if 'reduced' in pool_str:
                pdicts = (
                    pool_dict('skip', s1) if s1==1 else pool_dict(_ptype, s1, ps=32, rt=1/4, nh=2, ct=_ctype, wn=True),
                    pool_dict('skip', s2) if s2==1 else pool_dict(_ptype, s2, ps=16, rt=1/4, nh=4, ct=_ctype, wn=True),
                    pool_dict('skip', s3) if s3==1 else pool_dict(_ptype, s3, ps=8, rt=1/4, nh=8, ct=_ctype, wn=True),
                    pool_dict('skip', s4) if s4==1 else pool_dict(_ptype, s4, ps=4, rt=1/4, nh=16, ct=_ctype, wn=True),
                    pool_dict('skip', s5) if s5==1 else pool_dict(_ptype, s5, ps=4, rt=1/4, nh=32, ct=_ctype, wn=True),
                    pool_dict('skip', s6) if s6==1 else pool_dict(_ptype, s6, ps=2, rt=1/4, nh=64, ct=_ctype, wn=True),
                    pool_dict('skip', s7) if s7==1 else pool_dict(_ptype, s7, ps=2, rt=1/4, nh=128, ct=_ctype, wn=True),
                )
