	...
	def __init__(self, cfg, block, layers, num_classes=1000, zero_init_residual=False,
		...
		if pool_params == None:
			...
			for i, ch in enumerate(chs):
				res_layers.append(nn.Sequential(OrderedDict([
									('conv_layer', self._make_layer(cfg.conv2d, block, ch, layers[i], stride=strides[i]))
									])))
		else:
			for i, ch in enumerate(chs):
				...
				if pool2d==None:
					res_layers.append(nn.Sequential(OrderedDict([
										('conv_layer', self._make_layer(cfg.conv2d, block, ch, layers[i], stride=stride))
										])))
				else:
					...
					res_layers.append(nn.Sequential(OrderedDict([
										('conv_layer', self._make_layer(cfg.conv2d, block, ch, layers[i], stride=1)),
										('pooling_layer', pool2d(ch*block.expansion, kernel_size=ksize, stride=stride, patch_size=psize, embed_dim=embed_dim, num_heads=num_heads))
										])))
...
def _resnet(arch, cfg, block, layers, pretrained, pth_file, **kwargs):
	...
	if pretrained:
		...
		state_dict = {}
		for pk in pretrained_resnet.keys():
			pk_lst = pk.split('.')
			if 'fc' in pk:
				continue
			if 'layer' in pk_lst[0]:
				layer_ord = int(pk_lst[0][5:])-1
				prefix = ["res_layers", str(layer_ord), 'conv_layer']
				suffix = pk_lst[1:]
				mk =  ".".join(prefix + suffix)
			else:
				mk = pk

			pv = pretrained_resnet[pk]
			mv = model_dict[mk]

			if mv.shape!=pv.shape:
				print('Mismatched Shape!', pk, '=>', mk)
				state_dict[mk] = pv.expand(mv.shape)
			else:
				print('Matched Shape!', pk, '=>', mk)
				state_dict[mk] = pv
...
def resnet18(cfg, pretrained=False, pth_file=None, **kwargs):
	...
	"""
	return _resnet('resnet18', cfg, BasicBlock, [2, 2, 2, 2], pretrained, pth_file,
				   **kwargs)
