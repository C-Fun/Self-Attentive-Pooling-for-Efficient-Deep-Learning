...
class LinearSpike(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, input, threshold, scale):
        
        ctx.save_for_backward(input)
        out = torch.zeros_like(input).cuda()
        out[input > 0] = float(threshold*scale*1.5)
        return out
    ...
    def backward(ctx, grad_output):
        ...
        return grad*grad_input, grad_input, None
...
class BasicBlock(nn.Module):
    ...
    def forward(self, dic):
        ...
        t 				= dic['t']
        leak			= dic['leak']
        scale_factor_y  = dic['scale_factor_y']
        i               = dic['scale_pos']
        spike_count     = dic['spike_count']
        ...
        delta_mem 		= self.conv1(inp)
        mem[pos] 		= getattr(leak, 'l'+str(pos)) *mem[pos] + delta_mem #+ (getattr(threshold, 't'+str(pos))/6)
        mem_thr 		= (mem[pos]/getattr(threshold, 't'+str(pos))) - 1.0
        rst 			= getattr(threshold, 't'+str(pos)) * (mem_thr>0).float()
        mem[pos] 		= mem[pos] - rst
        ...
        #relu1
        out 			= act_func(mem_thr, getattr(threshold, 't'+str(pos)), scale_factor_y[i])
        i += 1
        spike[pos] 		= spike[pos].masked_fill(out.bool(),t-1)
        spike_count[pos][out.bool()] 	= spike_count[pos][out.bool()] + 1
        out_prev  		= out.clone()
        ...
        delta_mem 		= self.conv2(out_prev) + self.identity(inp)
        mem[pos+1] 		= getattr(leak, 'l'+str(pos+1))*mem[pos+1] + delta_mem #+(getattr(threshold, 't'+str(pos+1))/6)
        mem_thr 		= (mem[pos+1]/getattr(threshold, 't'+str(pos+1))) - 1.0
        rst 			= getattr(threshold, 't'+str(pos+1)) * (mem_thr>0).float()
        ...
        #relu2
        out 			= act_func(mem_thr, getattr(threshold, 't'+str(pos+1)), scale_factor_y[i])
        i += 1
        spike[pos+1]	= spike[pos+1].masked_fill(out.bool(),t-1)
        spike_count[pos+1][out.bool()] 	= spike_count[pos+1][out.bool()] + 1
        out_prev  		= out.clone()
        ...
        #pdb.set_trace()
        return out_prev, spike_count[pos], spike_count[pos+1]
...
class RESNET_SNN(nn.Module):
	...
	def __init__(self, resnet_name, activation='Linear', labels=10, timesteps=75, leak=1.0, default_threshold=1.0, alpha=0.5, beta=0.035, dropout=0.2, dataset='CIFAR10'):
		...
		self.spike 			= {}
		self.spike_count    = {}
		with open('sample_new_factor_x_resnet20_cifar100_2','rb') as f:
			self.scale_factor = pickle.load(f)
		with open('sample_new_factor_y_resnet20_cifar100_2','rb') as f:
  ...
  #                               )
		...
		self.layer1, self.pos1 		= self._make_layer(block, 64, cfg[self.resnet_name][0], stride=1, dropout=self.dropout, pos=pos)
		self.layer2, self.pos2 		= self._make_layer(block, 128, cfg[self.resnet_name][1], stride=2, dropout=self.dropout, pos=self.pos1)
		self.layer3, self.pos3		= self._make_layer(block, 256, cfg[self.resnet_name][2], stride=2, dropout=self.dropout, pos=self.pos2)
		self.layer4, self.pos4 		= self._make_layer(block, 512, cfg[self.resnet_name][3], stride=2, dropout=self.dropout, pos=self.pos3)
	...
	def threshold_update(self, scaling_factor=1.0, thresholds=[]):
		...
		for pos in range(len(self.pre_process)):
			if isinstance(self.pre_process[pos],nn.Conv2d):
				#if thresholds:
				self.threshold.update({'t'+str(pos): nn.Parameter(torch.tensor(getattr(self.threshold, 't'+str(pos))*self.scale_factor[i]))})
				i += 1

		pos = len(self.pre_process)
		total_res = 16
		for l in range(total_res):
			self.threshold.update({'t'+str(pos): nn.Parameter(torch.tensor(getattr(self.threshold, 't'+str(pos+l))*self.scale_factor[i]))})
			i += 1
			#pos +=1
	...
	def neuron_init(self, x):
		...
		for l in range(len(self.pre_process)):
			...
			if isinstance(self.pre_process[l], nn.Conv2d):
				...
				self.mask[l] = self.pre_process[l](torch.ones(self.mem[l-1].shape))
			elif isinstance(self.pre_process[l], nn.MaxPool2d):
				
				self.width 	= self.width//self.pre_process[l].kernel_size
				self.height = self.height//self.pre_process[l].kernel_size 

		pos = len(self.pre_process)
		layer_drop = nn.Dropout(self.dropout)
		for i in range(1,5):
			layer = self.layers[i]
			self.width = self.width//layer[0].conv1.stride[0]
			self.height = self.height//layer[0].conv1.stride[0]
			for index in range(len(layer)):
				self.mem[pos] = torch.zeros(self.batch_size, layer[index].conv1.out_channels, self.width, self.height)
				self.spike[pos] = torch.ones(self.mem[pos].shape)*(-1000)
				self.spike_count[pos] = torch.zeros(self.mem[pos].shape)
				...
				self.mask[pos] = layer_drop(torch.ones(self.mem[pos].shape))
				pos = pos + 1
				self.mem[pos] = torch.zeros(self.batch_size, layer[index].conv2.out_channels, self.width, self.height)
				self.spike[pos] = torch.ones(self.mem[pos].shape)*(-1000)
				self.spike_count[pos] = torch.zeros(self.mem[pos].shape)				
				pos = pos + 1
		...
		for l in range(len(self.classifier)):
			if isinstance(self.classifier[l],nn.Linear):
				...
				self.spike_count[pos+l] = torch.zeros(self.mem[pos+l].shape)			
	...
	def forward(self, x, find_max_mem=False, max_mem_layer=0, percentile=99.7):
		...
		for t in range(self.timesteps):
			...
			for l in range(len(self.pre_process)):
				...
				if isinstance(self.pre_process[l], nn.Conv2d):
					...
					self.mem[l] 	= getattr(self.leak, 'l'+str(l)) *self.mem[l] + self.pre_process[l](out_prev) #+ (getattr(self.threshold, 't'+str(l))/6)
					mem_thr 		= (self.mem[l]/getattr(self.threshold, 't'+str(l))) - 1.0
					rst 			= getattr(self.threshold, 't'+str(l)) * (mem_thr>0).float()
					self.mem[l] 	= self.mem[l] - rst
					out 			= self.act_func(mem_thr, getattr(self.threshold, 't'+str(l)), self.scale_factor_y[k])
					k += 1
			...
			for i in range(1,5):
				...
				layer = self.layers[i]
				for index in range(len(layer)):
					out_prev, self.spike_count[pos], self.spike_count[pos+1] = layer[index]({'out_prev':out_prev.clone(), 'pos': pos, 'act_func': self.act_func, 'mem':self.mem, 'spike':self.spike, 'mask':self.mask, 'threshold':self.threshold, 't': t, 'leak':self.leak, 'scale_factor_y':self.scale_factor_y, 'scale_pos':k, 'spike_count':self.spike_count})
					pos = pos+2
		...
		if len(self.classifier)>1:
			return self.mem[pos+l+1], self.spike_count
		else:
			return self.mem[pos], self.spike_count
